...
from src.datasets import VQADataset_Synonym
...
def main(args):
    ...
    transform = transforms.Compose([
        ...
        transforms.RandomRotation(degrees=10),  # 過度な回転を避ける
        transforms.ColorJitter(brightness=0.1, contrast=0.1, saturation=0.1, hue=0.05),  # 色調整の範囲を限定する
        transforms.ToTensor(),
        transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])
    ])
    data_dir = Path("/data/daichi/VQA")
    train_dataset = VQADataset_Synonym(df_path=data_dir/"train.json", image_dir=data_dir/"train", answer_dict_path=args.answer_dict_path, transform=transform)
    test_dataset = VQADataset_Synonym(df_path=data_dir/"valid.json", image_dir=data_dir/"valid", answer_dict_path=args.answer_dict_path, transform=transform, answer=False)
